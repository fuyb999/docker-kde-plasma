...
function start() {
    ...
    source /opt/aura-dock/bin/venv-set.sh serviceportal
    source /opt/aura-dock/bin/venv-set.sh selkies
    ...
    "$SERVICEPORTAL_VENV_PYTHON" /opt/aura-dock/fastapi/logviewer/main.py \
        -p $LISTEN_PORT \
        -r 5 \
        -s "${SERVICE_NAME}" \
    ...
    if [[ ${COTURN_ENABLED,,} == "true" ]]; then
        export SELKIES_TURN_HOST="${SELKIES_TURN_HOST:-$(/opt/aura-dock/bin/external-ip-address)}"
