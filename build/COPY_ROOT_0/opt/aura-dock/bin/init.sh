...
function init_count_quicktunnels() {
    if [[ ${CF_QUICK_TUNNELS,,} == "false" ]]; then
        ...
        export CF_QUICK_TUNNELS_COUNT=$(grep -l "QUICKTUNNELS=true" /opt/aura-dock/bin/supervisor-*.sh | wc -l)
...
init_sync_opt() {
    ...
    if [[ $WORKSPACE_MOUNTED = "true" ]]; then
        ...
        for item in "${path_array[@]}"; do
            opt_dir="/opt/${item}"
            if [[ ! -d $opt_dir || $opt_dir = "/opt/" || $opt_dir = "/opt/aura-dock" ]]; then
                continue
            fi
...
function init_debug_print() {
    if [[ -n $DEBUG ]]; then
        ...
        printf "\n--------------------------------------------\n"
        printf "/opt/aura-dock/etc/environment.sh...\n\n"
        cat /opt/aura-dock/etc/environment.sh
        printf "\n--------------------------------------------\n"
        printf ".bashrc...\n\n"
        cat /root/.bashrc
        printf "\n---------- END DEBUG INFO---------- \n\n\n"
